...
{
    public class Line : Point                     //继承point类
    {
        ...
        public double Xpoint
        {
            get
            {
                return basepoint[0];               //only readable
            }
        }
        public double Ypoint
        {
            get
            {
                return basepoint[1];                   //only readable
            }
        }
        public double AccessAngle
        {
            get
            {
                return angle;                   //only readable
            }
        }    //只有Xpoint,YPoint,AccessAngle是外部代码能访问到
        ...
        //角度的标准化
        {   //
            //  NormalAngle=Math.Atan(Math.Tan(Angle));这样也可以实现角度的标准化，此外不同于C/C++，
            //  C#支持对double类型的数据取模
            //
            double NormalAngle;
            ...
            {
                NormalAngle = Angle - Convert.ToInt32(Angle) + Convert.ToInt32(Angle) % 180 - 180 * ((Convert.ToInt32(Angle) % 180) > 90 ? 1 : 0);                                                                               //转化成合理角度
            }
            else
                NormalAngle = Angle - Convert.ToInt32(Angle) + Convert.ToInt32(Angle) % 180 + 180 * ((Convert.ToInt32(Angle) % 180) < -90 ? 1 : 0);
            return NormalAngle;
        }
        ...
        {
            this.angle = AngleNormalize(Angle);
            //对给定的非标准角度规范化
            if (this.angle != 90)
            {
        ...
        public Line(double X1, double Y1, double Angle)
            : base(X1, Y1)   //给定一个点横纵坐标及角度
        {
            this.angle = AngleNormalize(Angle);
            //对给定的非标准角度规范化
            if (this.angle != 90)
            ...
            : base(A.X, A.Y)                                                                            //给定两个点
        {
            ...
            {
                this.angle = Math.Atan((B.Y - A.Y) / (B.X - A.X)) / Math.PI * 180;    
                //规范化angle
            }
            if (angle != 90)
            {
        ...
        {
            double total_x = 0, total_y = 0, sq_x = 0, total_xy = 0;  
            //分别为x之和，y之和，x的平方之和，xy乘积之和
