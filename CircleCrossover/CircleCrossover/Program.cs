...
namespace CircleCrossover
{
    ...
    class Program
    {
        public static double[] Fun_cross(double x1, double y1, double x2, double y2, double r1, double r2)               //定义求交点函数Fun_cros
        {
            ...
            if (y1 != y2)
            {
                ...
                x_1 = (-b + System.Math.Sqrt(b * b - 4 * a * c)) / (2 * a);                                    //求解交点坐标
        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                ans = Fun_cross(C1.X, C1.Y, C2.X, C2.Y, C1.Radius, C2.Radius);
                Console.WriteLine("交点的坐标是({0},{1}),({2},{3})", ans[0], ans[1], ans[2], ans[3]);
            }
        }
    }
}
